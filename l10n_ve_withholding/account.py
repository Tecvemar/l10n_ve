...
class account_period(osv.osv):
    ...
    def find_fortnight(self, cr, uid, date=None, context=None):
        ...
        period_ids = self.find(cr, uid, dt=date, context=context)
        period_ids = self.search(
            cr, uid, [('special', '=', False), ('id', 'in', period_ids)])
        if not period_ids:
            raise osv.except_osv(
                _('Error !'),
                _('No period defined for this date: %s !\n' +
                  'Please create a fiscal year.') % date)

        fortnight = time.strptime(date, '%Y-%m-%d').tm_mday > 15
