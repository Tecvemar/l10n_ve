...
class txt_iva(osv.osv):
    ...
    _columns = {
        ...
        'type':fields.boolean('Retention Suppliers?',required=True,readonly=True,states={'draft':[('readonly',False)]}, help="Select the type of retention to make"),
        'date_start': fields.date('Begin Date',required=True,readonly=True,states={'draft':[('readonly',False)]}, help="Begin date of period"),
        'date_end': fields.date('End date', required=True,readonly=True,states={'draft':[('readonly',False)]}, help="End date of period"),
        ...
        'amount_total_ret':fields.function(_get_amount_all,method=True, digits=(16, 2), readonly=True, string='Withholding total amount', help="Monto Total Retenido", multi='all'),
        'amount_total_base':fields.function(_get_amount_all,method=True, digits=(16, 2), readonly=True, string='Taxable total amount', help="Total de la Base Imponible", multi='all'),
    }
    ...
    def action_generate_lines_txt(self,cr,uid,ids,context=None):
        ...
        if txt_brw.type:
            draft_ids = voucher_obj.search(cr,uid,[('date_ret','>=',txt_brw.date_start),('date_ret','<=',txt_brw.date_end),('period_id','=',txt_brw.period_id.id),('state','=','draft'),('type','in',['in_invoice','in_refund'])])
            ...
            draft_ids = voucher_obj.search(cr,uid,[('date_ret','>=',txt_brw.date_start),('date_ret','<=',txt_brw.date_end),('period_id','=',txt_brw.period_id.id),('state','=','draft'),('type','in',['out_invoice','out_refund'])])
        ...
        for voucher in voucher_obj.browse(cr,uid,voucher_ids):
            acc_part_id = rp_obj._find_accounting_partner(voucher.third_party_id or voucher.partner_id)
