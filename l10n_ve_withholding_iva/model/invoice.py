...
class account_invoice(osv.osv):
    ...
    def action_cancel(self, cr, uid, ids, *args):
        ...
        obj_wh_iva = self.pool.get('account.wh.iva')
        obj_wh_line = self.pool.get('account.wh.iva.line')
        for item in self.browse(cr, uid, ids, context={}):
            wh_line_ids = obj_wh_line.search(
                cr, uid, [('invoice_id', '=', item.id)])
            if wh_line_ids:
                raise osv.except_osv(
                    _('Error!'),
                    _('You can not cancel an invoice associated with ' +
                      'retention (IVA)'))
            wh_line_brw = obj_wh_line.browse(cr, uid, wh_line_ids, context={})
            wh_iva_brw = wh_line_brw and wh_line_brw[0].retention_id
            if wh_iva_brw and wh_iva_brw.state == 'draft' and \
                    not wh_iva_brw.number and \
                    len(wh_iva_brw.wh_lines) == 1:
                # Autodelete wh_iva
                obj_wh_iva.send_workflow_signal(
                    cr, uid, wh_iva_brw.id, 'cancel')
                obj_wh_iva.unlink(cr, uid, [wh_iva_brw.id], context={})
    ...
    def onchange_partner_id(self, cr, uid, ids, type, partner_id,
        ...
        if partner_id:
            ...
            if not part.property_account_receivable or not part.property_account_payable:
                raise osv.except_osv(_('Error!'),_('You must check accounting settings for partner: %s')%part.name)
        else:
            part = False
        ...
        if part:
            ...
            data[data.keys()[0]]['group_wh_iva_doc'] = part.group_wh_iva_doc
    ...
    def action_wh_iva_create(self, cr, uid, ids, context=None):
        ...
        for inv_brw in self.browse(cr, uid, ids, context=context):
            if inv_brw.wh_iva_id:
                ...
                #~ resolver problema de el if de abajo cuando se debe crear una retencion nueva (nueva quincena)
                if (inv_brw.company_id.wh_consolidate or inv_brw.group_wh_iva_doc) and fortnight_wh_id:
                    ...
                    data = {'wh_lines': [(4, ret_line_id)]}
                    ret = wh_iva_obj.browse(cr, uid, ret_id, context=context)
                    if ret.date_ret < inv_brw.date_invoice:
                        data.update({'date_ret': inv_brw.date_invoice,
                                     'date': inv_brw.date_invoice})
                    wh_iva_obj.write(cr, uid, ret_id, data, context=context)
                else:
    ...
    def get_fortnight_wh_id(self, cr, uid, ids, context=None):
        ...
        for inv_brw in self.browse(cr, uid, ids, context=context):
            ...
            ## Must compare this invoice 'date_invoice' fortnight with date (date_invoice) of
            ## the stored invoices in the withholding doc
            for acc_wh_brw in wh_iva_obj.browse(cr, uid, acc_wh_ids, context=context):
                inv_in_wh_fortnight = False
                for inv_in_wh in acc_wh_brw.wh_lines:
                    inv_in_wh_fortnight = per_obj.find_fortnight(cr, uid,
                                                          inv_in_wh.invoice_id.date_invoice,
                                                          context=context)
                if inv_in_wh_fortnight == inv_fortnight:
                    res.append(
                        inv_in_wh_fortnight == inv_fortnight
    ...
    def create_new_wh_iva(self, cr, uid, ids, ret_line_id, context=None):
        ...
        res = list()
        for inv_brw in self.browse(cr, uid, ids, context=context):
            per_id = per_obj.find(cr, uid, inv_brw.date_invoice)
            ...
            ret_iva = {
                'name':_('WH IVA - ORIGIN %s')%(inv_brw.number),
                'type': wh_type,
                'account_id': acc_id,
                'partner_id': inv_brw.partner_id.id,
                'period_id': per_id and per_id[0] or inv_brw.period_id.id,
...
#            return True
    ...
    def check_withholdable(self, cr, uid, ids, context=None):
        ...
        if inv_brw.type == 'in_refund' and inv_brw.parent_id:
            ...
            return  per_obj.find_fortnight(cr, uid, date=dt_refund, context=context) == \
                    per_obj.find_fortnight(cr, uid, date=dt_invoice, context=context)
        return False
    ...
    def button_generate_wh_doc(self, cr, uid, ids, context=None):
        ...
        view_id = self.pool.get('ir.ui.view').\
            search(cr, uid, [('name', '=', 'account.wh.iva.form.customer')])
        context.update({
            'invoice_id': inv_brw.id,
            'type': inv_brw.type,
            'default_partner_id': inv_brw.partner_id.id,
