...
class fiscal_book(osv.osv):
    ...
    def _get_wh(self, cr, uid, ids, field_names, arg, context=None):
        ...
        for fb_brw in self.browse(cr, uid, ids, context=context):
            for fbl_brw in fb_brw.fbl_ids:
                if fbl_brw.iwdl_id:
                    if fbl_brw.emission_date in periods:
                        emission_period = periods[fbl_brw.emission_date]
                    else:
                        try:
                            emission_period = period_obj.find(
                                cr, uid, fbl_brw.emission_date, context=context)
                        except:
                            emission_period = None
                            logger.info(
                                'Date out of fiscal year: %s' %
                                (fbl_brw.emission_date))
                            pass
                        periods[fbl_brw.emission_date] = emission_period
                    if emission_period and \
                            emission_period[0] == fb_brw.period_id.id:
    ...
    def _compute_purchase_book_totals(self, cr, uid, fb, context=None):
        total = {'exe': {'base': 0, 'tax': 0},
                 'im': {'gen': {'base': 0, 'tax': 0},
                        'adi': {'base': 0, 'tax': 0},
                        'red': {'base': 0, 'tax': 0},
                        },
        ...
        for item in fb.fbl_ids:
            if item.emission_date < min_date:  # adjust
                total['aju']['base'] += item.vat_general_base + \
                    item.vat_reduced_base + item.vat_additional_base
                total['aju']['tax'] += item.vat_general_tax + \
                    item.vat_reduced_tax + item.vat_additional_tax
            else:  # in period
                total['exe']['base'] += item.vat_sdcf + item.vat_exempt
                total[item.type]['gen']['base'] += item.vat_general_base
                total[item.type]['adi']['base'] += item.vat_additional_base
                total[item.type]['red']['base'] += item.vat_reduced_base
                total[item.type]['gen']['tax'] += item.vat_general_tax
                total[item.type]['adi']['tax'] += item.vat_additional_tax
                total[item.type]['red']['tax'] += item.vat_reduced_tax
                total['tot']['base'] += item.vat_sdcf + item.vat_exempt + \
                    item.vat_general_base + item.vat_additional_base + \
                    item.vat_reduced_base
                total['tot']['tax'] += item.vat_general_tax + \
                    item.vat_additional_tax + item.vat_reduced_tax
        return [total]

    def _compute_sale_book_totals(self, cr, uid, fb, context=None):
        total = {'exe': {'base': 0, 'tax': 0},
                 'exp': {'base': 0, 'tax': 0},
                 'int': {'gen': {'base': 0, 'tax': 0},
                         'adi': {'base': 0, 'tax': 0},
                         'red': {'base': 0, 'tax': 0},
                        },
                 'tot': {'base': 0, 'tax': 0},
                 'to2': {'base': 0, 'tax': 0},
                 'aju': {'base': 0, 'tax': 0},
                 'aj2': {'base': 0, 'tax': 0},  # New seniat form update 16-03
                 'ret': {'base': 0, 'tax': 0},
                 }
        if not fb:
            return [total]
        ...
        for item in fb.fbl_ids:
            if item.emission_date < min_date:  # adjust
                ...
                if item.type == 'ex':
                    total['exp']['base'] += item.vat_exempt
                else:
                    total['exe']['base'] += item.vat_sdcf + item.vat_exempt
                    total['int']['gen']['base'] += item.vat_general_base
                    total['int']['adi']['base'] += item.vat_additional_base
                    total['int']['red']['base'] += item.vat_reduced_base
                    total['int']['gen']['tax'] += item.vat_general_tax
                    total['int']['adi']['tax'] += item.vat_additional_tax
                    total['int']['red']['tax'] += item.vat_reduced_tax
                total['tot']['base'] += item.vat_sdcf + item.vat_exempt + \
                    item.vat_general_base + item.vat_additional_base + \
                    item.vat_reduced_base
                total['tot']['tax'] += item.vat_general_tax + \
                    item.vat_additional_tax + item.vat_reduced_tax
                if item.doc_type != 'AJST':
                    total['ret']['base'] += item.get_wh_vat
                else:
                    total['aj2']['base'] += item.get_wh_vat
        total['to2']['base'] = total['tot']['base'] + total['aju']['base']
        total['to2']['tax'] = total['tot']['tax'] + total['aju']['tax']
        return [total]
    ...
    def update_book_wh_iva_lines(self, cr, uid, fb_id, context=None):
        ...
        unrelated_iwdl_ids = iwdl_obj.search(
            cr, uid, [('fb_id', '=', fb_id), ('id', 'not in', iwdl_ids)],
            context=context)
        if unrelated_iwdl_ids:
            iwdl_obj.write(cr, uid, unrelated_iwdl_ids, {'fb_id': False},
                           context=context)
        return True
    ...
    def get_order_criteria_adjustment(self, cr, uid, type, context=None):
        return type == 'sale' \
            ...
            or 'emission_date asc, invoice_number asc, wh_number asc'
    ...
    def update_book_lines(self, cr, uid, fb_id, context=None):
        ...
        iwdl_ids = [x.id for x in fb_brw.iwdl_ids]
        showed_iwdl_ids = []
        for inv_brw in fb_brw.invoice_ids:
            ...
            iwdl_ok = iwdl_id in iwdl_ids
            values = {
                'invoice_id': inv_brw.id,
                'emission_date': (inv_brw.date_document or inv_brw.date_invoice) or False,
                'accounting_date': inv_brw.date_invoice or False,
                'imex_date': imex_invoice and inv_brw.dua_form_id.date or False,
                ...
                'affected_invoice': (doc_type == "N/DE" or doc_type == "N/CR") \
                                    and (inv_brw.parent_id and
                                            (fb_brw.type == 'sale' and inv_brw.parent_id.number or inv_brw.parent_id.supplier_invoice_number) or False) \
                                    or False,
                ...
                'doc_type': doc_type,
                'void_form': self.get_void_form(cr, uid, inv_brw, fb_brw),
                'fiscal_printer': inv_brw.fiscal_printer or False,
                'z_report': inv_brw.z_report or False,
                'custom_statement': inv_brw.dua_form_id and inv_brw.dua_form_id.name or False,
                }
            if inv_brw.wh_iva_id and values['emission_date'] == inv_brw.wh_iva_id.date_ret:
                iwdl_brw = iwdl_obj.browse(cr, uid, iwdl_id, context=context) if iwdl_ok else False
                values.update({
                ...
                showed_iwdl_ids.append(iwdl_id)
            if imex_invoice and inv_brw.dua_form_id:
                # if imex ivnoice compute values from customs tax forms
                #~ values.update({'emission_date': inv_brw.date_invoice or False})
                imex_inv_ids = []
                totals = {
                    'emission_date': inv_brw.date_invoice or False,
                    'total_with_iva': 0,
                    'vat_sdcf': 0,
                    'vat_general_base': 0,
                    'vat_general_tax': 0,
                    'vat_general_rate': 0,
                    'doc_type': 'F/IMP' if fb_brw.type == 'purchase' else 'F/EXP',
                    }
                for f86 in inv_brw.dua_form_id.customs_form_ids:
                    for tax in f86.cfl_ids:
                        if tax.tax_code.vat_detail:
                            for l in tax.imex_tax_line:
                                totals['vat_general_base'] += l.base_amount
                                totals['vat_general_tax'] += l.tax_amount
                                totals['vat_general_rate'] += l.tax_id.amount * 100
                                imex_inv_ids.append(l.imex_inv_id.id)
                totals['total_with_iva'] += totals['vat_general_base'] + totals['vat_general_tax']
                if fb_brw.type == 'purchase' and inv_brw.id not in imex_inv_ids:
                    values = False
                else:
        ...
        missing_iwdl_ids = list(set(iwdl_ids) - set(showed_iwdl_ids))
        if missing_iwdl_ids:
            ...
            for iwdl_brw in iwdl_obj.browse(cr, uid, missing_iwdl_ids,
                ...
                values = {
                    ...
                    'partner_name': rp_brw.name or 'N/A',
                    'partner_vat': rp_brw.vat and rp_brw.vat[2:] or 'S/R',
...
#~
    ...
    def link_book_lines_and_taxes(self, cr, uid, fb_id, context=None):
        ...
        for fbl in self.browse(cr, uid, fb_id, context=context).fbl_ids:
            if fbl.invoice_id:
                ...
                if fbl.invoice_id and fbl.invoice_id.state == 'cancel':
                    sign = 0
                context.update({'date': fbl.accounting_date})
                if fbl.invoice_id.currency_id.id != \
                        fbl.invoice_id.company_id.currency_id.id:
                    amount_untaxed = currency_obj.compute(
                        cr, uid, fbl.invoice_id.currency_id.id,
                        fbl.invoice_id.company_id.currency_id.id,
                        fbl.invoice_id.amount_untaxed, context)
                else:
                    amount_untaxed = fbl.invoice_id.amount_untaxed
                amount_field_data = {
                    'total_with_iva': amount_untaxed * sign,
                    'vat_sdcf': 0.0, 'vat_exempt': 0.0}
                taxes = fbl.type in ['im','ex'] \
                    and fbl.invoice_id.imex_tax_line \
                    or fbl.invoice_id.tax_line
                #~ Only for untaxed invoices
                if not taxes and not amount_field_data['vat_exempt']:
                    amount_field_data['vat_exempt'] = amount_field_data['total_with_iva']
                for ait in taxes:
                    if ait.tax_id:
                        ...
                        data.append((0, 0, {'fb_id': fb_id,
                                            'fbl_id': False,
                                            'ait_id': ait.id}))
                if fbl.doc_type == 'F/EXP':
                    amount_field_data['vat_exempt'] = amount_field_data['total_with_iva']
                if fbl.doc_type != 'F/IMP':
    ...
    def get_doc_type(self, cr, uid, inv_id=None, iwdl_id=None, cf_id=None,
                     fb_id=None, fb_browse=None, context=None):
        ...
        if inv_id:
            ...
            if (inv_brw.type in ["in_invoice"] and inv_brw.parent_id) \
                ...
                res = "N/CR" if inv_brw.type == "in_refund" else "N/DB"
            ...
            res = 'F/EXP' if fb_brw.type == 'sale' else 'F/IMP'
        return res

    def get_void_form(self, cr, uid, inv_brw, fb_brw, context=None):
    ...
    def is_invoice_imex(self, cr, uid, inv_id, context=None):
        ...
        inv_obj = self.pool.get('account.invoice')
        inv_brw = inv_obj.browse(cr, uid, inv_id, context=context)
        return inv_brw.expedient and True or False
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if 'article_number' in vals:
            #~ avoid invalid article_number writes
            if vals.get('article_number') not in ('75', '76', '77', '78'):
                vals.pop('article_number')

        res = super(fiscal_book, self).write(cr, uid, ids, vals, context)
        return res

    def test_cancel(self, cr, uid, fb_ids, context=None):
        fb_ids = isinstance(fb_ids, (int, long)) and [fb_ids] or fb_ids
        obj_sum = self.pool.get('fiscal.summary')
        for item in self.browse(cr, uid, fb_ids, context={}):
            fld_name = 'fb_%s_id' % item.type
            sum_ids = obj_sum.search(cr, uid, [(fld_name, '=', item.id)])
            for sum_brw in obj_sum.browse(cr, uid, sum_ids, context=context):
                if sum_brw.state != 'draft':
                    raise osv.except_osv(
                        _('Error!'),
                        _('Can\'t cancel an declared fiscal book'))
                else:
                    obj_sum._clear_lines(cr, uid, [sum_brw.id], context)
        return True


class fiscal_book_lines(osv.osv):
    ...
    def _get_wh_vat(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for fbl_brw in self.browse(cr, uid, ids, context=context):
            sign = 1 if fbl_brw.doc_type not in ('N/CR', 'RN/C') else -1
            if fbl_brw.invoice_id and fbl_brw.invoice_id.state == 'cancel':
                    sign = 0
            if fbl_brw.iwdl_id:
    ...
    def _compute_vat_rates(self, cr, uid, ids, field_name, arg, context=None):
        ...
        for item in self.browse(cr, uid, ids, context=context):
            res[item.id] = {
                ...
                'check_total': round(abs(item.total_with_iva - (
                    item.vat_sdcf + item.vat_exempt +
                    item.vat_reduced_base + item.vat_reduced_tax +
                    item.vat_general_base + item.vat_general_tax +
                    item.vat_additional_base + item.vat_additional_tax)), 2)
                }
            if item.fb_id.state != 'draft':
                #~ Don't show warnings in posted book
                res[item.id]['check_total'] = 0
        return res
    ...
    _columns = {
        ...
        'check_total': fields.function(
            _compute_vat_rates, method=True, type='float',
            string='Check total', multi='all',
            help="Field to validate book line data Ok == total_with_iva"),
    }
...
class fiscal_book_taxes(osv.osv):
...
class fiscal_book_taxes_summary(osv.osv):
