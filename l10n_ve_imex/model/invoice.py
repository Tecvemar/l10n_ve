...
class inherited_invoice(osv.osv):
    ...
    _columns = {
        'dua_form_id': fields.many2one(
            'dua.form', 'DUA form', required=False, ondelete='restrict'),
        'customs_form_id': fields.function(
            _get_imex_invoices, method=True, readonly=True,
    ...
    def write(self, cr, uid, ids, vals, context=None):
        if 'expedient' in vals and not vals['expedient']:
            vals.update({'dua_form_id': 0})
        res = super(inherited_invoice, self).write(
...
class inheried_account_invoice_tax(osv.osv):
    ...
    _columns = {
        ...
        'partner_id': fields.related('imex_inv_id','partner_id', type='many2one', relation='res.partner', string='Supplier',store=False, readonly=True),
    ...
    def on_change_invoice_id(self, cr, uid, ids, imex_inv_id):
        res = {}
        if imex_inv_id:
            obj_inv = self.pool.get('account.invoice')
            inv = obj_inv.browse(cr, uid, imex_inv_id)
