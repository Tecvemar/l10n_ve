...
class islr_wh_doc(osv.osv):
    ...
    _sql_constraints = [
      ('number_uniq', 'unique (number,id)', 'The number has already assigned, you cannot assigned it twice!')
    ]
    ...
    def check_income_wh(self, cr, uid, ids, context=None):
        ...
        res = {}
        if not obj.date_uid or not obj.date_ret:
            raise osv.except_osv(_('Error!'), _('Must indicate: Accounting date and (or) Voucher Date'))
        if obj.type in ('in_invoice', 'in_refund'):
            cr.execute('''
                select max(id) from islr_xml_wh_doc
                where state not in ('cancel','draft') and
                      period_id = %s and
                      company_id = %s''' % (obj.period_id.id, obj.company_id.id))
            if cr.fetchone()[0]:
                raise osv.except_osv(
                    _('Error!'),
                    _('Must indicate a accounting period > %s') %
                    obj.period_id.name)
    ...
    def validate_date_ret(self, cr, uid, ids, context=None):
        ids = isinstance(ids, (int, long)) and [ids] or ids
        obj = self.browse(cr, uid, ids[0], context=context)
        if not obj.number and obj.type in ('in_invoice', 'in_refund'):
    ...
    def action_done(self, cr, uid, ids, context=None):
        ...
        context = context or {}
        ids = isinstance(ids, (int, long)) and [ids] or ids
        self.validate_date_ret(cr, uid, ids, context=context)
        ...
        for item in self.browse(cr, uid, ids, context={}):
            for iwdi in item.invoice_ids:
                ixwl_ids = []
                for ixwl in iwdi.islr_xml_id:
                    ixwl_ids.append(ixwl.id)
                obj_ixwl.write(cr, uid, ixwl_ids, {'date_ret': item.date_ret},
                               context=context)
        return True
    ...
    def action_cancel(self, cr, uid, ids, context={}):
        ...
        wil_obj = self.pool.get('islr.wh.doc.invoices')
        for wh_doc in self.browse(cr,uid,ids,context={}):
            if wh_doc.type in ('in_invoice', 'in_refund'):
                cr.execute('''
                select max(id) from islr_xml_wh_doc
                where state not in ('cancel','draft') and
                      period_id = %s and
                      company_id = %s''' % (wh_doc.period_id.id,
                                            wh_doc.company_id.id))
                if cr.fetchone()[0]:
                    raise osv.except_osv(
                        _('Error!'),
                        _('You can\'t cancel a withholding while the period is declared'))
            for line in wh_doc.invoice_ids:
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        if unlink_ids:
            super(islr_wh_doc, self).unlink(cr, uid, unlink_ids, context=context)
        return True
...
class islr_wh_doc_invoices(osv.osv):
    ...
    _columns = {
        ...
        'move_id': fields.many2one('account.move', 'Accounting entries', ondelete='restrict',
                                   readonly=True, help="The move of this entry line."),
    }
    ...
    def load_taxes(self, cr, uid, ids, context=None):
        ...
        if not ret_line.invoice_id or ret_line.islr_wh_doc_id.state != 'draft':
            return True
    ...
    def _get_rate(self, cr, uid, concept_id, residence, nature, context):
        ...
        raise osv.except_osv(
            _('Error!'),
            _('Not valid ISLR withholding concept selected for the current partner'))
        return ()
    ...
    def _get_country_fiscal(self, cr, uid, partner_id, context=None):
        ...
        context = context or {}
        address = [addr for addr in partner_id.address if addr.type == 'invoice']
        if address and address[0].country_id:
            return address[0].country_id.id
        else:
            raise osv.except_osv(_('Invalid action !'), _(
