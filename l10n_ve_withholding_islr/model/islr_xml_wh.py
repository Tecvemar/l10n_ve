...
class islr_xml_wh_doc(osv.osv):
    ...
    _columns = {
        ...
        'period_id':fields.many2one('account.period','Period',required=True, help="Period when the accounts entries were done",readonly=True, states={'draft': [('readonly', False)]}),
    ...
    def action_confirm1(self, cr, uid, ids, context={}):
        ...
        obj_iwd = self.pool.get('islr.wh.doc')
        for item in self.browse(cr, uid, ids, context={}):
            draft_ids = obj_iwd.search(
                cr, uid, [('period_id', '=', item.period_id.id),
                          ('state', 'not in', ('done', 'cancel')),
                          ('type', 'in', ('in_invoice', 'in_refund'))])
            if draft_ids:
                raise osv.except_osv(
                    _('Error!'),
                    _('Can\'t confirm XML lines while exists withholding ' +
                      'documents in process for period'))
    ...
    def _xml(self, cr,uid,ids):
        ...
        for id in ids:
            ...
            GROUP BY partner_vat,control_number,porcent_rete,concept_code,invoice_number,account_invoice_id,date_ret
            ORDER BY date_ret, partner_vat, control_number, concept_code'''%(wh_brw.period_id.id,', '.join([str(i.id) for i in wh_brw.xml_ids]))
            cr.execute(sql)
            ...
            for line in xml_lines:
                ...
                d = date_ret.split('-')
                fecha_operacion = '%s/%s/%s' % (d[2], d[1], d[0])
                ...
                SubElement(detalle, "FechaOperacion").text = fecha_operacion
...
class islr_xml_wh_line(osv.osv):
    ...
    _columns = {
        ...
        'date_ret': fields.date('Date Operation', readonly=True),
